...
def part_two(number):
    ...
    while True: ## I have to traverse the array in a circular pattern. I think I can do it based on whats around the number. We can push the position a w a y from other numbers we find around it, that way we don't have bunch of conditionals. Okay actually no. I think there is easier way.
        adjaciency_list = []
        layer += 2
        ...
        for perimeter_count in range(0, perimeter):
            if perimeter_count == 0:
                pos[0] += 1
            elif perimeter_count < perimeter//4:
                pos[1] += 1
            elif perimeter_count < perimeter//2:
                pos[0] -= 1
            elif perimeter_count < perimeter//4 + perimeter//2:
                pos[1] -= 1
            else:
                pos[0] += 1

            for x_transf, y_transf in transforms:
                adjaciency_list.append(matrix[pos[1] + y_transf][pos[0] + x_transf])
