...
def empties(inp: list[list]) -> tuple[list, list]:
    ...
    rows = sorted(list(set(index if '#' not in inp[index] else -1 for index in range(len(inp)))))[1:]
    cols = sorted(list(set(index if '#' not in list(zip(*inp))[index] else -1 for index in range(len(inp[0])))))[1:]
    return(rows, cols)
...
def distance(gal1: tuple[int, int], gal2: tuple[int, int], expands: tuple[list, list], modifier: int) -> int:
    """function to return distance between two galaxies"""
    r_delta = abs(gal1[0] - gal2[0]) + sum(modifier for expand in expands[0] if min(gal1[0], gal2[0]) < expand < max(gal1[0], gal2[0]))
    c_delta = abs(gal1[1] - gal2[1]) + sum(modifier for expand in expands[1] if min(gal1[1], gal2[1]) < expand < max(gal1[1], gal2[1]))
...
def a(inp: str) -> int:
    ...
    return sum(distance(galaxies[i], galaxies[j], expands, 1) for i in range(len(galaxies)) for j in range(i + 1, len(galaxies)) )
...
def b(inp: str, modifier: int) -> int:
    ...
    return sum(distance(galaxies[i], galaxies[j], expands, modifier-1) for i in range(len(galaxies)) for j in range(i + 1, len(galaxies)) )
